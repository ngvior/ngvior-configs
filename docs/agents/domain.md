...
- **`CONTEXT-MAP.md`** at the repo root — if this file ever appears, the repo has been
  promoted to multi-context. It then wins over the single-context rules above: read it,
  then read each per-context `CONTEXT.md` relevant to the topic plus any context-scoped
  `<context>/docs/adr/`.
